...
public class Thread implements Runnable {
	/** 这里只看一些 常见的参数 */
	/** 线程名 */
	private volatile char name[];//线程名是不稳定的
	/** 优先级 */
	private int priority;//优先级 int类型
	/** 是否为守护线程 */
	private boolean daemon;//是否为守护线程   bool
	/** 线程要执行的目标任务 */
	private Runnable target; //目标任务为tartget
	/** 所属线程组 */
	private ThreadGroup group;//线程组?
	/** 类加载器 */
	private ClassLoader contextClassLoader;//classloader
	/**
	 * ThreadLocal 能为线程设置线程私有变量 就是通过下面这个threadLocals变量完成的，
	 * ThreadLocal的get/set方法就是通过操作 各个线程的 threadLocals 变量实现的。 (threadLocal调用的对象，实际为Thread类下的ThreadLocal下的threadMap)
     ...
     * 上面的意思是可以 类.thradLocalMap.get("线程名",name)？？ 
     *
	 */
	ThreadLocal.ThreadLocalMap threadLocals;
	ThreadLocal.ThreadLocalMap inheritableThreadLocals;
	/** 线程栈的大小 */
	private long stackSize;//线程栈的大小 JVM有关了
	/**
	 * Thread类定义了6个线程状态：New、Runnable、Blocked、Waiting、TimedWaiting、Terminated(终止)
	 * 实际上还会把 Runnable 再细分为 就绪(未抢到时间片) 和 运行中(抢到时间片)
     * 所以一共有7个状态 new runnable(ready running) blocked waiting TimeWaiting Terminated
	 */
	private volatile int threadStatus;//线程状态也是不稳定的
	/** 最小优先级 */
     ...
     * 通过Thread.threadStatus 来获取当前线程的状态
	 */
	 ...
	 */
	public Thread() {
		daemon = false;//默认不是守护线程
		stillborn = false;
		threadLocals = null;//默认没有threadLocals
		inheritableThreadLocals = null;
		threadStatus = 0;//默认为new
		blockerLock = new Object(); //用于阻塞的锁，锁同城为一个对象
		init(null, null, (new StringBuilder()).append("Thread-").append(nextThreadNum()).toString(), 0L);
	}

    //把实现了Runnable的类给new起来
	...
	private void init(ThreadGroup threadgroup, Runnable runnable, String name, long l,
		...
		if (threadgroup == null) {
			if (securitymanager != null)
				threadgroup = securitymanager.getThreadGroup();//securitymanager的组
			if (threadgroup == null)
				threadgroup = parent.getThreadGroup();//默认和父亲一组
		}
	...
	public synchronized void start() {
	    ...
	    group.add(this);//这里的group 就相当于队列的队伍
